...
//   const [productDetailsResult, availableSizeResult, gendersResult] =
...
//       {productDetailsData?.relatedProducts?.length > 0 && (
//         <AlsoLike products={productDetailsData.relatedProducts} />
//       )}
//     </>
//   );
...
// components/product-page/product-details.tsx
...
export default async function ProductDetailPage({ params }: ProductDetailProps) {
  const { id } = params; // params is now already resolved ✔
  ...
  const availableSizeData =
    availableSizeResult.status === 'fulfilled'
      ? availableSizeResult.value
      : [];
